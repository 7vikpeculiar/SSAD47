...
var summaryInterval = require('./summaryInterval');
...
new CronJob('0 0 4 * * *', function() {
  ...
  var job = queue.create('computeSummaryInterval', {}).attempts(5);
  job.save();
...
// Calculating sales summary for an organisation
queue.process('computeSummaryInterval', function(job,done){
  logger.info('computeSummaryInterval task called with id', job.id);
  summaryIntervalTask.computeSummaryInterval(job.data, function(err){
    logger.info('computeSummaryInterval task finished with id', job.id);
    done(err);
  })
});
