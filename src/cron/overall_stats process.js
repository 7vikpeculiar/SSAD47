...
var overallStatsTask = require('./overallStats');
...
, true, 'Asia/Kolkata');function (err) {
    logger.info('computeOr task findished with id', job.id);
    done(err);
  })
function (err) {
    logger.info('computeOr task findished with id', job.id);
...
new CronJob('0 53 18 * * *', function() {
  var job = queue.create('allOrganisationStats',{}).attempts(5);
  job.save();
}, function() { }
, true, 'Asia/Kolkata');
...
queue.process('computeOrganisationStats', function (job, done) {
  ...
  organisationStatsTask.allOrganisationStats(job.data, function (err) {
    logger.info('computeOrganisationStats task findished with id', job.id);
    done(err);
  })
});

// TO calculate overall stats at a company level
queue.process('allOrganisationStats', function (job, done) {
  logger.info('allOrganisationStats task called with job id:', job.id);

  overallStatsTask.allOrganisationStats(function (err) {
    logger.info('all organisation stats task finished with id:', job.id);
    done(err);
  })
});
